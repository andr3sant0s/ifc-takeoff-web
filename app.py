...
<progress id="bar" value="0" max="100" style="width:350px"></progress>
...
def convert_piso_for_description(ifc_piso):
    try:
        n=int(str(ifc_piso))
        m={1:"-02",2:"-01",3:"00",4:"Mezanine"}
        return m.get(n,str(n-4))
    except: 
        return str(ifc_piso)

def piso_sort_key_from_ifc(ifc_piso):
    try:
        n=int(str(ifc_piso))
        m={1:1,2:2,3:3,4:4}
        return m.get(n,5+(n-4))
    except: 
        return 999
...
def run_one(ifc_path, excel_path, output_path):
    ...
    for _,row in df.iterrows():
        ...
        for piso in sorted(by.keys(),key=piso_sort_key_from_ifc):
            q=round(by[piso],3)
            out.append([row["WBS"],f"{desc} - Piso {convert_piso_for_description(piso)}",uom,piso,q,cost,round(q*cost,3)])
    ...
    wv.append(["GlobalId","IfcClass","WBS","Reason"])

    for v in val: 
        wv.append([v["GlobalId"],v["IfcClass"],v["WBS"],v["Reason"]])
